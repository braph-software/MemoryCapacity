classdef GlobalMemoryCapacity < Measure
	...
	properties (Constant) % properties
		...
		MC_ALL_TRIALS = Measure.getPropNumber() + 4;
		MC_ALL_TRIALS_TAG = 'MC_ALL_TRIALS';
		MC_ALL_TRIALS_CATEGORY = Category.RESULT;
		MC_ALL_TRIALS_FORMAT = Format.CELL;
		
		MC_CALC = Measure.getPropNumber() + 5;
		MC_CALC_TAG = 'MC_CALC';
		MC_CALC_CATEGORY = Category.QUERY;
		MC_CALC_FORMAT = Format.CELL;
	end
	methods % constructor
	...
	methods (Static) % inspection
			...
			if nargin == 0
				prop_list = [ ...
					Measure.getProps() ...
						...
						GlobalMemoryCapacity.MC_ALL_TRIALS ...
						GlobalMemoryCapacity.MC_CALC ...
						];
				return
			end
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					...
					case GlobalMemoryCapacity.MC_ALL_TRIALS
						tag = GlobalMemoryCapacity.MC_ALL_TRIALS_TAG;
					case GlobalMemoryCapacity.MC_CALC
						tag = GlobalMemoryCapacity.MC_CALC_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				...
				case GlobalMemoryCapacity.MC_ALL_TRIALS
					prop_category = GlobalMemoryCapacity.MC_ALL_TRIALS_CATEGORY;
				case GlobalMemoryCapacity.MC_CALC
					prop_category = GlobalMemoryCapacity.MC_CALC_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				...
				case GlobalMemoryCapacity.MC_ALL_TRIALS
					prop_format = GlobalMemoryCapacity.MC_ALL_TRIALS_FORMAT;
				case GlobalMemoryCapacity.MC_CALC
					prop_format = GlobalMemoryCapacity.MC_CALC_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case GlobalMemoryCapacity.TRAINING_SAMPLES
					prop_description = 'TRAINING_SAMPLES (parameter, scalar) is the number of training samples (length of input time series).';
				case GlobalMemoryCapacity.TAU_MAX
					prop_description = 'TAU_MAX (parameter, scalar) is the maximum delay to be considered.';
				case GlobalMemoryCapacity.MC_ALL_TRIALS
					prop_description = 'MC_ALL_TRIALS (result, cell) is the global memory capacity at all trials.';
				case GlobalMemoryCapacity.MC_CALC
					prop_description = 'MC_CALC (query, cell) calculates memory capacity at specific settings of specified delay, ridge, and wieghted matrices.';
				...
				case GlobalMemoryCapacity.M
					prop_description = 'M (result, cell) is the average global memory capacity across all trials.';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case GlobalMemoryCapacity.TAU_MAX
					prop_default = 50;
				case GlobalMemoryCapacity.MC_ALL_TRIALS
					prop_default = Format.getFormatDefault(Format.CELL, GlobalMemoryCapacity.getPropSettings(prop));
				case GlobalMemoryCapacity.MC_CALC
					prop_default = Format.getFormatDefault(Format.CELL, GlobalMemoryCapacity.getPropSettings(prop));
				case GlobalMemoryCapacity.ELCLASS
		...
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				...
				case GlobalMemoryCapacity.TAU_MAX % __GlobalMemoryCapacity.TAU_MAX__
					check = Format.checkFormat(Format.SCALAR, value, GlobalMemoryCapacity.getPropSettings(prop));
				case GlobalMemoryCapacity.MC_ALL_TRIALS % __GlobalMemoryCapacity.MC_ALL_TRIALS__
					check = Format.checkFormat(Format.CELL, value, GlobalMemoryCapacity.getPropSettings(prop));
				case GlobalMemoryCapacity.MC_CALC % __GlobalMemoryCapacity.MC_CALC__
					check = Format.checkFormat(Format.CELL, value, GlobalMemoryCapacity.getPropSettings(prop));
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(m, prop, varargin)
			...
			switch prop
				case GlobalMemoryCapacity.MC_ALL_TRIALS % __GlobalMemoryCapacity.MC_ALL_TRIALS__
					rng_settings_ = rng(); rng(m.getPropSeed(GlobalMemoryCapacity.MC_ALL_TRIALS), 'twister')
